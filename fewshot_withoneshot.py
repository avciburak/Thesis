...
def main():
    ...
    # read image files
    groundtruth_image_path="/content/drive/MyDrive/group1/00000001.jpg"
    ground_truth_path="/content/drive/MyDrive/groundtruthgroup1_changed.txt"
    bounding_boxes_path="/content/drive/MyDrive/group1_4_changed.txt"
    groundtruth_image=Image.open(groundtruth_image_path)
    with open(ground_truth_path) as f:
        line = f.readline()
        coordinates=line.split(",")
        coordinates=[float(coordinate) for coordinate in coordinates]
        groundtruth_image=groundtruth_image.crop((coordinates[0],coordinates[1],coordinates[0]+coordinates[2],coordinates[1]+coordinates[3]))
    groundtruth_image.show()
    transform_PILToTensor = transforms.Compose([transforms.PILToTensor()])
    test_image_tensor=transform_PILToTensor(groundtruth_image).to(torch.float64)
    ...
    with open(bounding_boxes_path) as f:
        ...
        while frame_finished:
            ...
            if raw_coordinates[0]=="1":
                bounding_box_number+=1
                box=(float(raw_coordinates[1]),float(raw_coordinates[2]),float(raw_coordinates[1])+float(raw_coordinates[3]),float(raw_coordinates[2])+float(raw_coordinates[4]))
                bounding_box_coordinates.append(box)
            else:
                break
    bounding_box_coordinates[0]=(100.0,100.0,184.0,184.0)
    boundingbox_image_tensors=[]
    for boxes in bounding_box_coordinates:
        boundingbox_image=groundtruth_image.crop(boxes)
        boundingbox_image_tensor=transform_PILToTensor(boundingbox_image).to(torch.float64)
        #print(boundingbox_image_tensor.size())
        boundingbox_image_tensors.append(boundingbox_image_tensor)
    ...
    relation_list=[]
    for image_tensor in boundingbox_image_tensors:
        sample_images=image_tensor.unsqueeze(0)
    ...
    # calculate features
        ...
        test_features = feature_encoder(Variable(test_image_tensor.unsqueeze(0).to(torch.float32)).cuda(GPU)) # 20x64
