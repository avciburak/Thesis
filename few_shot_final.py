...
def main():
    ...
    # read image files
    image_path="/content/drive/MyDrive/group1/00000001.jpg"
    ground_truth_path="/content/drive/MyDrive/groundtruthgroup1"
    bounding_boxes_path="/content/drive/MyDrive/group1_4"
    groundtruth_image=Image.open(image_path)
    ...
    with open(bounding_boxes_path) as f:
        ...
        while frame_finished:
            ...
            if raw_coordinates[0]=="1":
                ...
                box=[raw_coordinates[1],raw_coordinates[2],raw_coordinates[1]+raw_coordinates[3],raw_coordinates[2]+raw_coordinates[4]]
                bounding_box_coordinates.append(box)
            else:
                break
    cropped_bounding_boxes_tensors=[]
    for boxes in bounding_box_coordinates:
        cropped_bounding_boxes_tensors.append(transform_PILToTensor(groundtruth_image.crop(boxes)).to(torch.float64))
    ...
    SAMPLE_NUM_PER_CLASS=bounding_box_number
    sample_images=cropped_bounding_boxes_tensors.unsqueeze(0).to(torch.float64)
    # calculate features
