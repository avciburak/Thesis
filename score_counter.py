...
#    groundtruth_folders.sort(key=get_frame_number)
    ...
    f=open('/content/drive/MyDrive/iou&scores.txt', 'w')
...
#        groundtruth_image_tensor=transform_ToTensor(groundtruth_image).to(torch.float32)
    ...
    # to form a 100x128 matrix for relation network
            ...
            print(support_folder+" | "+bb+" | "+str((relations[0].tolist()[0]))+" | "+str(iou))
            
            f.write(support_folder+" | "+bb+" | "+str((relations[0].tolist()[0]))+" | "+str(iou)+"\n")
    f.close()
    c.close()
