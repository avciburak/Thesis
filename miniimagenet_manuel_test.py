...
def main():
    ...
    # read image files
    groundtruth_image_path="/content/drive/MyDrive/miniimagenet_images/n0153282900000005.jpg"
    test_images_path="/content/drive/MyDrive/miniimagenet_images/test_images"
    groundtruth_image=Image.open(groundtruth_image_path)
    transform_PILToTensor = transforms.Compose([transforms.PILToTensor()])
    groundtruth_image_tensor=transform_PILToTensor(groundtruth_image).to(torch.float64)
    os.path=test_images_path
    tensor_sequence=[]
    for image in os.listdir():
        test_image=Image.open(image)
        test_image_tensor=transform_PILToTensor(test_image).to(torch.float64)
        tensor_sequence.append(test_image_tensor)
    ...
    for tensor in tensor_sequence:
        sample_images=tensor.unsqueeze(0)
    # calculate features
