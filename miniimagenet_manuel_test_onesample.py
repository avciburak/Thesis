...
def main():
    ...
    # read image files
    groundtruth_image_path="/content/drive/MyDrive/miniimagenet_test_image/bb_3.jpg"
    ...
    for image in os.listdir(test_images_path):
        ...
        test_image_tensor=transform_ToTensor(test_image).to(torch.float32)
    ...
    # calculate features
        sample_features = feature_encoder(Variable(tensor.unsqueeze(0)).to(torch.float32).cuda(GPU)) # 5x64
        sample_features = sample_features.view(CLASS_NUM,SAMPLE_NUM_PER_CLASS,FEATURE_DIM,19,19)
        sample_features = torch.sum(sample_features,1).squeeze(1)
        test_features = feature_encoder(Variable(groundtruth_image_tensor.unsqueeze(0).to(torch.float32)).cuda(GPU)) # 20x64
